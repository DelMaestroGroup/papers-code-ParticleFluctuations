"""We note that the code for comparing to the J-V model works with 1/K as K compared to the definitions of the manuscript."""
...
function rho_rho_corr_canonical(x;
    ...
    term_smooth = (K / (2π^2)) * d2dx2_logabs_theta1(z, q, L; tol=tol, maxn=maxn)

    θ1, _, _ = theta1_and_derivs(z, q; tol=tol, maxn=maxn)

    τ = im * β * v_s / L
    θ1p0 = 2 * etaDedekind(τ)^3
